...
def chat():
    ...
    reply = generate_ai_response(message, language_mode, history, user_id=user_id)
...
def chat_stream():
    ...
    def generate():
        ...
        for chunk in generate_ai_response_stream(message, language_mode, history, user_id=user_id):
            full_reply += chunk
            yield chunk
...
def upload_file():
    ...
    if file:
        ...
        # Add to user-specific RAG if PDF or DOCX
        if filename.lower().endswith(('.pdf', '.docx')):
            add_document_to_user_index(user_id, filepath)
