...
def upload_file():
    ...
    def generate_progress():
        ...
        try:
            ...
            print(f"Client disconnected during upload of file {file_id}")
            raise
            
    def on_close():
        if not upload_state['success']:
            print(f"Cleaning up unsuccessful file upload {file_id}: success={upload_state['success']}")
            try:
                conn = get_db_connection()
                ...
                print(f"Error cleaning up database entry for file {file_id}: {db_err}")
            if os.path.exists(filepath):
                try:
                    os.remove(filepath)
                except Exception as fs_err:
                    print(f"Error removing physical file {filepath} on cleanup: {fs_err}")
            
    resp = Response(generate_progress(), mimetype='text/plain')
